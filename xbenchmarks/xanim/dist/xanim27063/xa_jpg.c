...
xaULONG jpg_read_DHT()
{
  ...
  { xaLONG i,index,count;
    ...
    len -= 17;
    if (count > 256)
	{ fprintf(stderr,"JJ: DHT bad count %d\n",count); return(xaFALSE); }
...
DEBUG_LEVEL1 fprintf(stderr,"JJ: marker %x\n",jpg_marker);
	...
	DEBUG_LEVEL1 fprintf(stderr,"JJ: skipped marker %x\n",jpg_marker);
	break;
    } /* end of switch */
  } /* end of forever */
}
...
xaULONG jpg_read_RST_marker()
{
  ...
  {
    DEBUG_LEVEL1 fprintf(stderr,"JJ: RST marker %x found\n",jpg_marker);
    return(xaTRUE);
  }
  else
  {
    fprintf(stderr,"JJ: NON-restart marker found %x\n",jpg_marker);
...
{ jpg_h_bbuf = 0;  jpg_h_bnum = 0;			\
  ...
  if (jpg_marker)							\
  { DEBUG_LEVEL1 fprintf(stderr,"  jpg_marker(%x)\n",jpg_marker);	\
    if (jpg_marker == M_EOI) { jpg_saw_EOI = xaTRUE; return(xaTRUE); }	\
...
if (jpg_marker)							\
{ DEBUG_LEVEL1 fprintf(stderr,"  jpg_marker(%x)\n",jpg_marker);	\
  if (jpg_marker == M_EOI) { jpg_saw_EOI = xaTRUE; \
           /* return(xaTRUE); */ }	\
  else if ( !((jpg_marker >= M_RST0) && (jpg_marker <= M_RST7)))	\
  {fprintf(stderr,"JPEG: unexp marker(%x)\n",jpg_marker); return(xaFALSE);}\
  else { \
...
xaULONG gray;
{
  ...
  mcu_cols = (width  + 7) / 8;
  mcu_rows = (height + 7) / 8;
  DEBUG_LEVEL1 fprintf(stderr,"111111 begin MCUS(%d,%d)\n",mcu_cols,mcu_rows);
  jpg_marker = 0x00;
