...
bpred_dir_lookup(struct bpred_dir_t *pred_dir, /* branch dir predictor inst */
                 ...
                 enum md_opcode prev)          /* preceding instruction's opcode */
                                               // md_opcode is defined in machine.h
{
  ...
  case BPred2bit:
    ...
    break;
  case BPredOpcode:
    p = &pred_dir->config.bimod.table[BIMOD_HASH(pred_dir, prev)];
    break;
  case BPredTaken:
  case BPredNotTaken:
    break;
...
bpred_lookup(struct bpred_t *pred,                  /* branch predictor instance */
             ...
             enum md_opcode prev,                   /* opcode of previous instruction */
                                                     ...
                                                     * used on mispredict recovery */
{
  ...
  case BPredOpcode:
    ...
    {
      dir_update_ptr->pdir1 =
          bpred_dir_lookup(pred->dirpred.bimod, 0, prev);
    }
    break;
  case BPredTaken:
    return btarget;
